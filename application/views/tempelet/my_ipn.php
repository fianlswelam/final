...
if (strpos($curl_result, "VERIFIED") !== false) {
	...
	mail("mohamedsaad2085@gmail.com", "IPN interaction not verified", "$req", "From: mohamedsaad2085@gmail.com" );
	exit();
}
...
if ($receiver_email != "mohamedsaad2085@gmail.com") {
	$message = "Investigate why and how receiver email is wrong. Email = " . $_POST['receiver_email'] . "\n\n\n$req";
    mail("mohamedsaad2085@gmail.com", "Receiver Email is incorrect", $message, "From: mohamedsaad2085@gmail.com" );
    exit(); // exit script
}
// Check number 2 ------------------------------------------------------------------------------------------------------------
...
if ($numRows > 0) {
    ...
    mail("mohamedsaad2085@gmail.com", "Duplicate txn_id in the IPN system", $message, "From: mohamedsaad2085@gmail.com" );
    exit(); // exit script
} 
// Check number 4 ------------------------------------------------------------------------------------------------------------
...
//Insert grossAmount + Elraseeed el7aly into db
$sql = mysql_query("SELECT `amount_money` FROM `user` WHERE `payment_email`='$payer_email'")or die ("unable to execute the query on the user .");
while($row = mysqli_fetch_array($sql))
  {
	$amount_money=$row['amount_money'] ;
  }
  $total=$amount_money+$grossAmount;

$sql = mysql_query("UPDATE `user` SET  `amount_money`= $total WHERE `payment_email`='$payer_email'") or die ("unable to execute the query");
...
mail("mohamedsaad2085@gmail.com", "NORMAL IPN RESULT YAY MONEY!", $req, "From: mohamedsaad2085@gmail.com");
?>